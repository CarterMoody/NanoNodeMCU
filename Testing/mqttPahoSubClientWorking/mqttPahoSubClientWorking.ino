...
//IPAddress mqtt_server = (192, 168, 1, 143);//MQTT server IP
IPAddress mqtt_server(192, 168, 1, 213);//MQTT server IP
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topic == "nodemcu/heartbeat")
  {
    Serial.println("topic is: nodemcu/heartbeat");
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      // ... and resubscribe
      client.subscribe("nodemcu/heartbeatAcknowledged");
    } else {
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    snprintf (msg, MSG_BUFFER_SIZE, "heartbeat");
    Serial.print("Publish message: ");
    Serial.println(msg);
    client.publish("nodemcu/heartbeat", "heartbeat");
  }
}
